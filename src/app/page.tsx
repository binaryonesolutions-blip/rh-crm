  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {(['draft','sent','confirmed','invoiced'] as QuoteStatus[]).map(s => (
          <div key={s} className="card cursor-pointer hover:border-blue-300 transition-colors"
               onClick={() => setFilter(s === filter as QuoteStatus ? 'all' : s)}>
            ...
            <tbody className="divide-y divide-gray-100">
              {visible.map(q => (
                <tr key={q.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{q.pi_number}</td>
